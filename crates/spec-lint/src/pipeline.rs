use spec_core::{LintDiagnostic, LintReport, QualityScore, Section, Severity, SpecDocument};
...
pub fn cross_check(docs: &[SpecDocument]) -> Vec<LintDiagnostic> {
    ...
    let mut spec_boundaries: Vec<(&str, Vec<(String, spec_core::BoundaryCategory)>)> = Vec::new();
        ...
        for j in (i + 1)..spec_boundaries.len() {
            ...
            for (text_a, cat_a) in bounds_a {
                for (text_b, cat_b) in bounds_b {
                    if text_a == text_b
                        && *cat_a != *cat_b
                        && ((*cat_a == spec_core::BoundaryCategory::Allow
                            && *cat_b == spec_core::BoundaryCategory::Deny)
                            || (*cat_a == spec_core::BoundaryCategory::Deny
                                && *cat_b == spec_core::BoundaryCategory::Allow))
                    {
